...
Clarinet.test({
  ...
  fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    gt.getBalance(user1.address).result.expectOk().expectUint(4000000);
    gt.getBalance(EXTENSIONS.CCD002_TREASURY_MIA_STACKING).result.expectOk().expectUint(0);
    ...
    gt.getBalance(user1.address).result.expectOk().expectUint(1800000); //4m-2.2m
